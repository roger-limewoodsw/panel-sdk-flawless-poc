...
import { getVideoExportSettingsName } from './create-settings';
import { registerTaskPath } from './utils.js';



let exportVideoToFile = function(mcapiclient, mob_id, mobType, destinationPath, fileName) {
    displayTextDebug("exportVideoToFile: " + mob_id + ", mobType: " + mobType + ", destinationPath: " + destinationPath + ", fileName: " + fileName);
    ...
    exportSettingsName = getVideoExportSettingsName();
    ...
    mcapiclient.exportFile(request, md, (err, response) => {
        if (err) {
            ...
            displayTextDebug("Export video file start successfully.");
            if (response && response.getHeader()) {
                ...
                registerTaskPath(header.getTaskId(),  destinationPath + "/" + fileName + ".mxf");
            } else {
